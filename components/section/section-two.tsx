...
export default function SectionTwo() {
  ...
  React.useEffect(() => {
    ...
    const onSelect = () => setContentSelectedIndex(contentApi.selectedScrollSnap());
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Contoh Carousel</CardTitle>
            <CardDescription>Sample konten umum (non-chart) 5 slide</CardDescription>
          </CardHeader>
          <CardContent>
            <Carousel
              className="w-full"
              opts={{ loop: true, align: "start" }}
              setApi={setContentApi}
              onMouseEnter={() => setContentPaused(true)}
              onMouseLeave={() => setContentPaused(false)}
              onTouchStart={() => setContentPaused(true)}
              onTouchEnd={() => setContentPaused(false)}
            >
              <CarouselContent>
                {/* Slide 1: Headline & deskripsi */}
                <CarouselItem>
                  <div className="h-[300px] rounded-md border border-border bg-card p-6">
                    <h3 className="text-xl font-semibold">Selamat datang di Dashboard Kinerja</h3>
                    <p className="mt-2 text-muted-foreground">
                      Gunakan carousel ini sebagai contoh konten informatif yang dapat menampilkan
                      pengumuman, highlight fitur, atau ringkasan performa.
                    </p>
                    <div className="mt-4 inline-flex items-center gap-2 rounded-md border border-border bg-background/60 px-3 py-2">
                      <TrendingUp className="size-4 text-primary" />
                      <span className="text-sm">Performa Q1 meningkat 12%</span>
                    </div>
                  </div>
                </CarouselItem>
                ...
                <CarouselItem>
                  <div className="h-[300px] rounded-md border border-border bg-card p-6">
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                      {mockData.kpiCards.slice(0, 3).map((kpi) => (
                        <div key={kpi.id} className={cn("rounded-md p-4", kpi.bgColor)}>
                ...
                {/* Slide 4: Daftar update */}
                <CarouselItem>
                  <div className="h-[300px] rounded-md border border-border bg-card p-6">
                    <h4 className="font-medium">Update Terbaru</h4>
                    <ul className="mt-3 list-disc pl-5 text-sm text-muted-foreground">
                      <li>Penambahan fitur dark mode di halaman login</li>
                      <li>Perbaikan layout footer agar konsisten di semua resolusi</li>
                      <li>Carousel dengan autoplay & indikator dot</li>
                      <li>Optimasi grafik untuk performa dan aksesibilitas</li>
                      <li>Penyesuaian warna tema untuk mode terang/gelap</li>
                    </ul>
                  </div>
                </CarouselItem>

                {/* Slide 5: Testimoni / kutipan */}
                <CarouselItem>
                  <div className="flex h-[300px] items-center justify-center rounded-md border border-border bg-card p-6 text-center">
                    <blockquote className="max-w-md">
                      <p className="text-lg">
                        “Dashboard ini membantu tim kami memantau kinerja harian dengan cepat
                        dan akurat.”
                      </p>
                      <footer className="mt-2 text-sm text-muted-foreground">— Kepala Bidang Operasional</footer>
                    </blockquote>
                  </div>
                </CarouselItem>
              </CarouselContent>
            ...
            {/* Indikator dot */}
            <div className="mt-3 flex justify-center gap-2">
              {contentScrollSnaps.map((_, idx) => (
                <button
                  key={idx}
                  aria-label={`Ke slide ${idx + 1}`}
                  onClick={() => contentApi?.scrollTo(idx)}
                  className={cn(
                    "h-2 w-2 rounded-full transition-colors",
                    idx === contentSelectedIndex ? "bg-primary" : "bg-muted-foreground/30"
                  )}
                />
              ))}
            </div>
