...
export default function ChartPerikananPondCultivation({ year, chartData }: Props) {

  const { theme, systemTheme } = useTheme();
  const currentTheme = theme === 'system' ? systemTheme : theme;
  const isDark = currentTheme === 'dark';

  const title = 'Data Produksi Budidaya Pembesaran Di Kolam';
  ...
  return (
    <CardComponent
      ...
      description={
        <>
          {/* Data Produksi Budidaya Pembesaran Di Kolam <br /> */}
            ...
            <Tabs defaultValue="all" className="flex flex-col gap-3">
              ...
              <div className="max-h-[60vh] overflow-y-auto rounded-md border">
                <TabsContent value="komoditi" className="p-0">
                  <TableMonthly dataFreq="monthly" year={year} unit="Ton" tableHeadColspan={'Jumlah Produksi Ikan Bulanan Tahun ' + year} tableFooterTotal="Total Seluruh Produksi Budidaya" dataChart={dataChart} />
                </TabsContent>
                <TabsContent value="kecamatan" className="p-0">
                  <TableDistrictly year={year} unit="Ton" tableHeadColspan={'Jumlah Produksi Ikan Komoditi Tahun ' + year} tableFooterTotal="Total Seluruh Produksi Budidaya" dataCommodities={dataChart?.data_commodities} dataChart={dataChart} />
                </TabsContent>
              </div>
            </Tabs>
          }
