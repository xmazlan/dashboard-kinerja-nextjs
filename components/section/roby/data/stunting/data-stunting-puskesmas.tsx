...
export default function DataStuntingPuskesmas({ onError }: Props) {
  const { data: apiData, isLoading: isLoadingApiData, error: masterError } =
    useStuntingSweeperPuskesmasData();
  ...
  const items: PuskesmasItem[] = (() => {
    const data = (apiData as unknown as { data?: PuskesmasItem[] })?.data;
      ...
      <CardComponent
        ...
        description={
          <>
            ...
            <div className="mt-1 text-xs">Puskesmas: {itemsSorted.length}</div>
          </>
        }
      >
        {isLoadingApiData ? (
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="text-[11px] md:text-xs font-semibold uppercase opacity-90">
                          Total Balita (Semua Puskesmas)
                        </div>
                        <div className="text-[10px] md:text-[11px] opacity-80">
                          Akumulasi ke semua puskesmas
                        </div>
                      </div>
                      <div
